...
A transcrição é **100% local**: o modelo Whisper (Large Turbo ~560 MB ou Large ~1,5 GB) roda dentro do navegador via Transformers.js + ONNX Runtime (WebGPU/WASM).

**Nenhum byte de áudio sai do seu computador**: sem telemetria, sem conta, sem servidor intermediário, sem chave de API — o áudio é processado no próprio Chrome e nunca é enviado para a rede.
...
3. Clique no ícone da VozClara → **Baixar Whisper** (baixa o Large Turbo uma vez, ~560 MB).
...
src/                  Site (TanStack Start + React 19 + Tailwind): demo e instalação
...
Produção em https://vozclara-whats.vercel.app/ via integração Git do Vercel: push na `main` publica automaticamente, cada PR ganha uma URL de preview. Nenhuma variável de ambiente é obrigatória.

Convenções: sem bundler na extensão (content scripts clássicos), Chrome 116+, EOL fixado via `.gitattributes`.

## Contribuindo

Issues e pull requests são bem-vindos. Para mudanças na extensão, rode `npm run check:pack` antes de abrir o PR — o zip em `public/` precisa acompanhar o código. Para dúvidas de comportamento, inclua a versão da extensão e o navegador.

## Privacidade em uma frase

Por padrão, tudo roda na sua máquina e nada é enviado para a rede.
